...
async function executeFunction(name: string, args: Record<string, string>) {
  if (name === 'book_appointment') {
    ...
    await prisma.client.upsert({
      where: { phone_business: { phone: phone, business: 'mg_masajes' } },
  ...
  try {
    ...
    if (senderPhone) {
      const client = await prisma.client.findUnique({
        where: { phone_business: { phone: senderPhone, business: 'mg_masajes' } }
      });
      if (client) {
    ...
    if (parts.length === 0 && message) {
      ...
      if (nameMatch) {
        ...
        const clients = await prisma.client.findMany({
          where: { name: { contains: name, mode: 'insensitive' }, business: 'mg_masajes' },
          take: 3,
        });
        if (clients.length > 0) {
    ...
    if (senderPhone) {
      try {
        await prisma.client.upsert({
          where: { phone_business: { phone: senderPhone, business: 'mg_masajes' } },
          update: {},
          create: { name: 'Cliente nuevo', phone: senderPhone, business: 'mg_masajes' },
        });
      } catch { /* ignore */ }
    }
    ...
    if (sessionId) {
      try {
        await prisma.chatMessage.createMany({
          data: [
            { business: 'mg_masajes', role: 'user', content: message, sessionId },
            { business: 'mg_masajes', role: 'assistant', content: reply, sessionId },
          ],
        });
      } catch { /* ignore */ }
    }
...
export { router as chatMgMasajesRoutes };
