...
const mockPrisma = prisma as unknown as {
  ...
  mgMasajesFinance: { findMany: ReturnType<typeof vi.fn>; create: ReturnType<typeof vi.fn>; update: ReturnType<typeof vi.fn>; delete: ReturnType<typeof vi.fn> };
...
describe('GET /api/mg_masajes/appointments', () => {
  it('returns empty array when no appointments', async () => {
    mockPrisma.appointment.findMany.mockResolvedValue([]);
    const res = await request(app).get('/api/mg_masajes/appointments').set('Authorization', authHeader('mg_masajes'));
...
describe('POST /api/mg_masajes/appointments', () => {
  ...
  it('returns 500 when prisma throws', async () => {
    ...
    const res = await request(app).post('/api/mg_masajes/appointments').set('Authorization', authHeader('mg_masajes')).send({
...
describe('PUT /api/mg_masajes/appointments/:id/status', () => {
  it('updates appointment status', async () => {
    mockPrisma.appointment.update.mockResolvedValue({ id: 'APT-1', status: 'completado' });
    const res = await request(app).put('/api/mg_masajes/appointments/APT-1/status').set('Authorization', authHeader('mg_masajes')).send({ status: 'completado' });
    expect(res.status).toBe(200);
  ...
  it('returns 500 on invalid id', async () => {
    ...
    const res = await request(app).put('/api/mg_masajes/appointments/FAKE/status').set('Authorization', authHeader('mg_masajes')).send({ status: 'completado' });
    expect(res.status).toBe(500);
  });
});
...
describe('GET /api/mg_masajes/finances', () => {
...
describe('POST /api/mg_masajes/finances', () => {
  it('creates a finance entry', async () => {
    ...
    mockPrisma.mgMasajesFinance.create.mockResolvedValue({ id: 'FIN-D-123', ...input });
    const res = await request(app).post('/api/mg_masajes/finances').set('Authorization', authHeader('mg_masajes')).send(input);
    expect(res.status).toBe(200);
  ...
  it('returns 500 when prisma throws', async () => {
    mockPrisma.mgMasajesFinance.create.mockRejectedValue(new Error('DB error'));
    const res = await request(app).post('/api/mg_masajes/finances').set('Authorization', authHeader('mg_masajes')).send({
...
describe('GET /api/mg_masajes/clients/search', () => {
  ...
  it('returns empty for no matches', async () => {
    mockPrisma.client.findMany.mockResolvedValue([]);
    const res = await request(app).get('/api/mg_masajes/clients/search?q=nonexistent').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
    expect(res.body).toEqual([]);
  });
});
...
describe('GET /api/mg_masajes/patients/:clientId/records', () => {
  ...
  it('returns 404 for non-existent client', async () => {
    ...
    const res = await request(app).get('/api/mg_masajes/patients/FAKE/records').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('Client not found');
  });
});

describe('POST /api/mg_masajes/patients/:clientId/records', () => {
  ...
  it('returns 404 for non-existent client', async () => {
    mockPrisma.client.findUnique.mockResolvedValue(null);
    const res = await request(app).post('/api/mg_masajes/patients/FAKE/records').set('Authorization', authHeader('mg_masajes')).send({
      date: '2026-04-05', reason: 'Test',
    });
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('Client not found');
...
describe('PUT /api/mg_masajes/patients/records/:id', () => {
  it('updates a patient record', async () => {
    ...
    const res = await request(app).put('/api/mg_masajes/patients/records/rec-1').set('Authorization', authHeader('mg_masajes')).send(update);
    expect(res.status).toBe(200);
  ...
  it('returns 500 on invalid id', async () => {
    ...
    const res = await request(app).put('/api/mg_masajes/patients/records/FAKE').set('Authorization', authHeader('mg_masajes')).send({ reason: 'X' });
    expect(res.status).toBe(500);
  });
});
...
describe('GET /api/mg_masajes/patients', () => {
  it('returns patients with record counts and last visit', async () => {
    ...
    const res = await request(app).get('/api/mg_masajes/patients').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(1);
...
describe('Damian validation', () => {
  ...
  it('PUT /appointments/:id/status returns 400 when status is empty', async () => {
    const res = await request(app).put('/api/mg_masajes/appointments/APT-1/status').set('Authorization', authHeader('mg_masajes')).send({});
    expect(res.status).toBe(400);
  });
  ...
  it('POST /clients returns 400 when phone is missing', async () => {
    const res = await request(app).post('/api/mg_masajes/clients').set('Authorization', authHeader('mg_masajes')).send({ name: 'Juan' });
    expect(res.status).toBe(400);
  });

  it('POST /patients/:clientId/records returns 400 when reason is missing', async () => {
    const res = await request(app).post('/api/mg_masajes/patients/c1/records').set('Authorization', authHeader('mg_masajes')).send({ date: '2026-04-05' });
    expect(res.status).toBe(400);
  });
});
...
describe('GET /api/mg_masajes/dashboard/today', () => {
  it('returns only today\'s appointments sorted by time', async () => {
    ...
    mockPrisma.appointment.findMany.mockResolvedValue(appointments);
    const res = await request(app).get('/api/mg_masajes/dashboard/today').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(2);
...
describe('GET /api/mg_masajes/dashboard/stale-patients', () => {
  it('returns patients with no records at all', async () => {
    ...
    const res = await request(app).get('/api/mg_masajes/dashboard/stale-patients').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(1);
...
describe('GET /api/mg_masajes/dashboard/appointments', () => {
  it('returns upcoming appointments (today and future) excluding cancelled', async () => {
    ...
    mockPrisma.appointment.findMany.mockResolvedValue(appointments);
    const res = await request(app).get('/api/mg_masajes/dashboard/appointments').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(2);
    expect(mockPrisma.appointment.findMany).toHaveBeenCalledWith(expect.objectContaining({
      where: {
...
describe('GET /api/mg_masajes/dashboard/monthly-income', () => {
  it('returns monthlyIncome summing current month income finances', async () => {
    ...
    const res = await request(app).get('/api/mg_masajes/dashboard/monthly-income').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
  ...
  it('returns 500 on DB error', async () => {
    mockPrisma.mgMasajesFinance.findMany.mockRejectedValue(new Error('DB error'));
...
describe('PUT /api/mg_masajes/clients/:id', () => {
  ...
  it('returns 500 when prisma throws', async () => {
    ...
    const res = await request(app).put('/api/mg_masajes/clients/c1').set('Authorization', authHeader('mg_masajes')).send({ name: 'Test' });
    expect(res.status).toBe(500);
  });
});
...
describe('PUT /api/mg_masajes/appointments/:id', () => {
  ...
  it('updates appointment fully', async () => {
    ...
    const res = await request(app).put('/api/mg_masajes/appointments/APT-1').set('Authorization', authHeader('mg_masajes')).send(fullAppointment);
    expect(res.status).toBe(200);
  ...
  it('returns 409 when time conflicts with existing appointment', async () => {
    ...
    const res = await request(app).put('/api/mg_masajes/appointments/APT-1').set('Authorization', authHeader('mg_masajes')).send({
...
describe('PUT /api/mg_masajes/finances/:id', () => {
  it('updates a finance entry', async () => {
    ...
    mockPrisma.mgMasajesFinance.update.mockResolvedValue(updated);
    const res = await request(app).put('/api/mg_masajes/finances/f1').set('Authorization', authHeader('mg_masajes')).send({ amount: 5000, description: 'Sesion Juan' });
    expect(res.status).toBe(200);
    expect(mockPrisma.mgMasajesFinance.update).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: 'f1' }
    }));
  });

  it('returns 500 when prisma throws', async () => {
    mockPrisma.mgMasajesFinance.update.mockRejectedValue(new Error('DB error'));
    const res = await request(app).put('/api/mg_masajes/finances/f1').set('Authorization', authHeader('mg_masajes')).send({ amount: 5000 });
    expect(res.status).toBe(500);
  });
});

describe('DELETE /api/mg_masajes/finances/:id', () => {
  it('deletes a finance entry', async () => {
    mockPrisma.mgMasajesFinance.delete.mockResolvedValue({});
    const res = await request(app).delete('/api/mg_masajes/finances/f1').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ success: true });
  });

  it('returns 500 when prisma throws', async () => {
    mockPrisma.mgMasajesFinance.delete.mockRejectedValue(new Error('DB error'));
    const res = await request(app).delete('/api/mg_masajes/finances/f1').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(500);
  });
});
...
describe('DELETE /api/mg_masajes/appointments/:id', () => {
  it('deletes an appointment and returns success', async () => {
    mockPrisma.appointment.delete.mockResolvedValue({});
    const res = await request(app).delete('/api/mg_masajes/appointments/APT-123').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ success: true });
...
describe('GET /api/mg_masajes/patients/:clientId/next-appointment', () => {
  it('returns the next upcoming appointment for the patient', async () => {
    ...
    const res = await request(app).get('/api/mg_masajes/patients/c1/next-appointment').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(200);
  ...
  it('returns 404 when client not found', async () => {
    ...
    const res = await request(app).get('/api/mg_masajes/patients/nonexistent/next-appointment').set('Authorization', authHeader('mg_masajes'));
    expect(res.status).toBe(404);
    expect(res.body.error).toBeDefined();
  });
...
describe('POST /api/mg_masajes/appointments - conflict detection', () => {
